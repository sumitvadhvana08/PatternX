...
`<pre>
<code>
for(int i=1;i&lt;=(2*x-1);i++){<br>

    for(int j=1;j&lt;=(2*x-1);j++){<br>
...
function p6(){
    ...
    for(let i=1;i<=(2*x-1);i++){

        let str="";

        for(let j=1;j<=(2*x-1);j++){
...
for(int i=n/2;i&lt;=n;i+=2){<br>

    for(int j=1;j&lt;n-i;j+=2){<br>
        System.out.print("  ");<br>
    }<br>
...
function p8(){
    ...
    if(x > 7){
        alert("Please Enter Number less than or equal to 6");
        return;
    }
